...
fn deploy_elf_path() -> PathBuf {
    if let Ok(dir) = std::env::var("CARGO_TARGET_DIR") {
        let path = PathBuf::from(dir).join("deploy/adapter_template.so");
        if path.exists() {
            return path;
        }
    }
    ...
    if let Some(workspace) = manifest.parent().and_then(|p| p.parent()) {
        let path = workspace.join("target/deploy/adapter_template.so");
        if path.exists() {
            return path;
        }
    }
    manifest.join("target/deploy/adapter_template.so")
}

fn setup() -> QuasarSvm {
    let elf = std::fs::read(deploy_elf_path())
        .unwrap_or_else(|e| panic!("build first: cd programs/adapter-template && quasar build ({e})"));
    QuasarSvm::new()
...
fn deposit_current_value_withdraw() {
    ...
    println!(
        "Template adapter — deposit CU: {}, value CU: {}, withdraw CU: {}",
